      ...
      <div className={styles.lotDetailGrid}>
        ...
        <div className={styles.infoSection}>
          ...
          <div className={styles.favoriteButtonWrap}>
            ...
            {/* Кнопка голосования */}
            <div className={styles.voteButtonRow}>
              <button
                onClick={handleToggleVote}
                disabled={isVoteLoading}
                className={`${styles.favoriteButtonDetail} ${styles.voteButtonDetail} ${isVoted ? styles.isActiveVote : ''}`}
                title="Проголосуйте, если хотите получить детальную оценку ИИ и инсайды от КУ для этого лота"
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill={isVoted ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path>
                </svg>
                {isVoted ? `Мой голос учтен (${votesCount})` : `Голосовать за детальный разбор (${votesCount})`}
              </button>
              <Link
                href="/how-it-works/ai-assessment"
                className={`${styles.voteHelpLink} ${isVoted ? styles.voteHelpLinkActive : ''}`}
                title="Как работает голосование за детальный разбор"
                aria-label="Как работает голосование за детальный разбор"
              >
